...
.cq.processConf:{[c]
    if [not count key `.r.configName; '"No config name specified in .r.configName"];
    if [not .r.configName in key c; '"No [",string[.r.configName],"] found for instance [",string[.cq.instance],"]"];
    conf:c[.r.configName];    
    ...
    if [count key `.r.processConf; .r.processConf[conf]];    
  };
